...
enum
{ T_ZONE, T_REGION, T_HOST, T_HUB, T_NODE };
                       ...
                       unsigned point, char *username )
{
  ...
  sprintf( nodenr, "%u:%u/%u.%u", zone % 65536, net % 65536, node % 65536, point % 65536 );
  ...
  else
  {
    for( cp = rev_username + strlen( rev_username );
         cp - rev_username < ENTRYLENGTH - ( signed ) strlen( nodenr ) - 1; cp++ )
    {
      *cp = ' ';
    }
  }
...
static void parse3d( char *ptr, unsigned int *zone, unsigned int *net, unsigned int *node )
{
  *zone = 0;
  while( isdigit( *ptr ) )
  {
                     ...
                     unsigned *zone, unsigned *net, unsigned *node )
{
  ...
  if( fgets( linebuf, sizeof( linebuf ), fin ) != NULL )
  {
    ...
    for( k = 0; ptr != NULL && k <= 4; k++ )
    {
      ...
      case F_POINTS4D:
        ...
        case 4:
          ...
          {
            username = ptr;
            rv = parsednode( fout, *zone, *net, *node, point, username );
          }
          break;
        }
        break;
    ...
    while( incomplete && fgets( linebuf, sizeof( linebuf ), fin ) != NULL )
    {
      l = strlen( linebuf );
      incomplete = ( l && linebuf[l - 1] != '\n' );
    }
