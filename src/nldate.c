/* $Id$
   ...
   Look a date in nodelist header.
*/
#include <stdio.h>
#include <ctype.h>
#include <string.h>
...
long parse_nodelist_date( char *filename )
{
  ...
  if( ncasecmp( cp, " -- Day number ", 15 ) || y < 1980 )
    goto invalid;
  ...
  {
    fclose( f );
    w_log( LL_ERROR, "Inconsistent nodelist header information in %s.", filename );
    return -1L;
  }
