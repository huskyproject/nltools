...
/*    w_log( 'X', "add_match()" ); */
  ...
  if( pnl == NULL && ( pnl = make_nlist(  ) ) == NULL )
    return 0;
...
nlist *find_nodelistfiles( char *path, char *base, int allowarc )
{
  ...
  while( ( dfile = husky_readdir( hdir ) ) != NULL )
  {
    ...
    if( l2 == l + 4 &&
        ...
        isdigit( dfile[l + 2] ) && isdigit( dfile[l + 3] ) )
    {
...
char *findNodelist( s_fidoconfig * config, unsigned int i )
{
  ...
  pnl = find_nodelistfiles( config->nodelistDir, config->nodelists[i].nodelistName, 0 );
